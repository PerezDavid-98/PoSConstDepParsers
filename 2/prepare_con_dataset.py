...
from stanza.utils.datasets.constituency.convert_en_masc import convert_en_masc
...
def process_en_masc(paths, dataset_name, *args):
    """
    Convert the MASC english treebank
    """
    lang, source = dataset_name.split("_", 1)
    assert lang == 'en'
    assert source == 'masc'

    input_path = os.path.join(paths["CONSTITUENCY_BASE"], "english", "MASC")
    output_dir = paths["CONSTITUENCY_DATA_DIR"]
    datasets = convert_en_masc(input_path)
